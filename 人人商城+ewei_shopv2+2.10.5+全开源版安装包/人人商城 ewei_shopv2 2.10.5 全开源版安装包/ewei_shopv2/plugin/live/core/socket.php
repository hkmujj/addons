...
class LiveSocket
{
	...
	{
		$this->log('msg_live', json_encode($data));
		...
		if ($data['type'] == 'update') {
			...
			if (($data['type'] == 'text') || ($data['type'] == 'image')) {
				...
				pdo_update('ewei_shop_live', $update, array('uniacid' => $data['uniacid'], 'id' => $data['roomid']));
	...
	'redpack'  => array('title' => $redpacktitle, 'id' => $redpackid),
	'msgid'    => pdo_insertid(),
	'roomid'   => $data['roomid']
	));
			}
						...
						if (redis()->exists($table_redpack)) {
							...
							if (0 < $redpacktotal) {
								...
								if (!empty($selfdata)) {
									...
									if (redis()->hLen($table_redpack_list) < $redpacktotal) {
	...
	protected function send($server = NULL, $fd = 0, $data = array())
	{
		...
		$data['pushstatus'] = json_encode($result);
		$this->log('msg_send', json_encode($data));
		return $result;
	}
	...
	protected function isManage($uid)
	{
		...
		if (strexists($uid, 'console')) {
			...
			if (is_array($user)) {
				if ($user[2] == 'founder') {
					if ($_W['config']['setting']['founder'] == $user[1]) {
						return true;
					}
				}

				$account = pdo_fetch('SELECT * FROM ' . tablename('uni_account_users') . ' WHERE uid=:uid AND role=:role AND uniacid=:uniacid', array(':uid' => $user[1], ':role' => $user[2], ':uniacid' => $user[3]));
