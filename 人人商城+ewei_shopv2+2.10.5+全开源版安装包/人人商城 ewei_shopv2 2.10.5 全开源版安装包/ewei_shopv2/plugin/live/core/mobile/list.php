...
class List_EweiShopV2Page extends PluginMobileLoginPage
{
	...
	public function get_list()
	{
		...
		if (!empty($total)) {
			$sql = 'SELECT id,title,thumb,livetime,subscribe,living FROM ' . tablename('ewei_shop_live') . "\r\n            \t\twhere 1 " . $condition . ' ORDER BY displayorder desc,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
			$list = pdo_fetchall($sql, $params);
			$list = set_medias($list, 'thumb');

			foreach ($list as $key => $value) {
				$list[$key]['subscribe'] = 0;
				...
				$list[$key]['subscribe'] = ($favorite['deleted'] == 1) || empty($favorite) ? 0 : 1;
			}
