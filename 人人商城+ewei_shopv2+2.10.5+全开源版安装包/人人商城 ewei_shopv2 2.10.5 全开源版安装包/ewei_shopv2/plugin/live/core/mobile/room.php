...
class Room_EweiShopV2Page extends PluginMobileLoginPage
{
	public function main()
	{
		...
		if (!empty($room['goodsid'])) {
			$room_goods = pdo_fetchall('select id,thumb,title,liveprice from ' . tablename('ewei_shop_goods') . ' where uniacid=:uniacid and id in (' . $room['goodsid'] . ') and islive > 0  ', array(':uniacid' => $_W['uniacid']));
		}
		...
		if (!empty($video)) {
			...
			if (!is_error($video_info) && !empty($video_info['hls_url'])) {
				...
				pdo_update('ewei_shop_live', array('thumb' => $poster), array('uniacid' => $uniacid, 'id' => $roomid));
			}
			else {
				$video = '';
			}
