...
class LiveModel extends PluginModel
{
	...
	public function handleRecords($roomid = 0, $manage = false)
	{
		...
		$records = redis()->lRange($table, 0, 100);
	...
	public function getLiveInfo($url = NULL, $type = 'auto')
	{
		...
		case 'douyu':
			...
			if (!empty($apiResult['data']['error'])) {
				return error(2, $apiResult['data']['data']);
			}
		...
		case 'huajiao':
			...
			$resultArr = array('status' => $feedInfo['paused'] == 'N' ? 1 : 0, 'poster' => $feedInfo['image'], 'hls_url' => 'http://qh.cdn.huajiao.com/live_huajiao_v2/' . $feedInfo['sn'] . '/index.m3u8');
			break;
		...
		case 'shuidi':
			preg_match('/.*view.html\\?sn=(\\d+)/is', $url, $matchs);
