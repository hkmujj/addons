...
class Cy163_customerserviceModuleSite extends WeModuleSite
{
	...
	public function doWebSetting()
	{
		...
		if ($op == 'post') {
			$id = intval($_GPC['id']);
			$data = array('weid' => $_W['uniacid'], 'title' => trim($_GPC['title']), 'istplon' => intval($_GPC['istplon']), 'unfollowtext' => trim($_GPC['unfollowtext']), 'followqrcode' => trim($_GPC['followqrcode']), 'sharetitle' => trim($_GPC['sharetitle']), 'sharedes' => trim($_GPC['sharedes']), 'sharethumb' => trim($_GPC['sharethumb']), 'kefutplminute' => intval($_GPC['kefutplminute']), 'bgcolor' => trim($_GPC['bgcolor']), 'defaultavatar' => trim($_GPC['defaultavatar']), 'issharemsg' => intval($_GPC['issharemsg']), 'isshowwgz' => intval($_GPC['isshowwgz']), 'sharetype' => intval($_GPC['sharetype']), 'mingan' => trim($_GPC['mingan']), 'temcolor' => trim($_GPC['temcolor']), 'candel' => intval($_GPC['candel']), 'copyright' => trim($_GPC['copyright']), 'isgrouptplon' => intval($_GPC['isgrouptplon']), 'grouptplminute' => intval($_GPC['grouptplminute']), 'isgroupon' => intval($_GPC['isgroupon']), 'footertext1' => trim($_GPC['footertext1']), 'footertext2' => trim($_GPC['footertext2']), 'footertext3' => trim($_GPC['footertext3']), 'footertext4' => trim($_GPC['footertext4']), 'qiniuaccesskey' => trim($_GPC['qiniuaccesskey']), 'qiniusecretkey' => trim($_GPC['qiniusecretkey']), 'qiniubucket' => trim($_GPC['qiniubucket']), 'qiniuurl' => trim($_GPC['qiniuurl']), 'isqiniu' => intval($_GPC['isqiniu']), 'httptype' => intval($_GPC['httptype']), 'istxfon' => intval($_GPC['istxfon']), 'ishowgroupnum' => intval($_GPC['ishowgroupnum']), 'chosekefutem' => intval($_GPC['chosekefutem']));
			if (!empty($id)) {
	...
	public function doWebChatlist()
	{
		...
		if ($operation == 'display') {
			...
			foreach ($cservicelist as $kk => $vv) {
				$cservicelist[$kk]['weidu'] = pdo_fetchcolumn("SELECT SUM(`guanlinum`) FROM " . tablename(BEST_FANSKEFU) . " WHERE kefuopenid = '{$vv['content']}'");
			}
			...
			if (!empty($kefuopenid)) {
				$allfkid = pdo_fetchall("SELECT fkid FROM " . tablename(BEST_CHAT) . " WHERE weid = {$_W['uniacid']} AND time > {$starttime} AND time < {$endtime}");
				$fkidarr = '(';
	...
	public function doWebJqr()
	{
		global $_GPC, $_W;
		$operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
		$allcservice = pdo_fetchall("SELECT * FROM " . tablename(BEST_CSERVICE) . " WHERE weid = {$_W['uniacid']} AND ctype = 1 ORDER BY displayorder ASC");
		if ($operation == 'display') {
			$jqrlist = pdo_fetchall("SELECT a.*,b.name FROM " . tablename(BEST_JQR) . " as a," . tablename(BEST_CSERVICE) . " as b WHERE a.weid = '{$_W['uniacid']}' AND a.kefuid = b.id");
			include $this->template('web/jqr');
		} elseif ($operation == 'post') {
			$id = intval($_GPC['id']);
			if (!empty($id)) {
				$jqr = pdo_fetch("SELECT * FROM " . tablename(BEST_JQR) . " WHERE id = :id AND weid = :weid", array(':id' => $id, ':weid' => $_W['uniacid']));
			}
			if (checksubmit('submit')) {
				if (empty($_GPC['title'])) {
					message('抱歉，请输入关键词！');
				...
				$data = array('weid' => $_W['uniacid'], 'title' => trim($_GPC['title']), 'huifu' => trim($_GPC['huifu']), 'kefuid' => intval($_GPC['kefuid']), 'type' => intval($_GPC['type']));
				if (!empty($id)) {
					pdo_update(BEST_JQR, $data, array('id' => $id, 'weid' => $_W['uniacid']));
				} else {
					pdo_insert(BEST_JQR, $data);
				}
				message('操作成功！', $this->createWebUrl('jqr', array('op' => 'display')), 'success');
			}
			include $this->template('web/jqr');
		} elseif ($operation == 'delete') {
			$id = intval($_GPC['id']);
			$jqr = pdo_fetch("SELECT id FROM " . tablename(BEST_JQR) . " WHERE id = {$id}");
			if (empty($jqr)) {
				message('抱歉，不存在该信息或是已经被删除！', $this->createWebUrl('jqr', array('op' => 'display')), 'error');
			}
			pdo_delete(BEST_JQR, array('id' => $id));
			message('删除成功！', $this->createWebUrl('jqr', array('op' => 'display')), 'success');
		}
	}
	...
	public function doWebCservicegroup()
	{
		...
		if ($operation == 'display') {
			...
			if (checksubmit('submit')) {
				...
				$data = array('weid' => $_W['uniacid'], 'name' => trim($_GPC['name']), 'typename' => trim($_GPC['typename']), 'thumb' => $_GPC['thumb'], 'displayorder' => intval($_GPC['displayorder']), 'qrtext' => trim($_GPC['qrtext']), 'qrcolor' => trim($_GPC['qrcolor']), 'qrbg' => trim($_GPC['qrbg']), 'cangroup' => intval($_GPC['cangroup']), 'ishow' => intval($_GPC['ishow']));
				if (!empty($id)) {
	...
	public function doMobileGroupchat()
	{
		...
		$referer = $_SERVER['HTTP_REFERER'];
		$id = intval($_GPC['id']);
		$hasgroup = pdo_fetch("SELECT * FROM " . tablename(BEST_GROUP) . " WHERE weid = {$_W['uniacid']} AND admin = '{$openid}' AND cservicegroupid = {$id}");
	...
	public function doMobileAutoaddgroup()
	{
		global $_W, $_GPC;
		$openid = $_W['fans']['from_user'];
		if (empty($openid)) {
		...
		$cgroupid = intval($_GPC['cgroupid']);
		$hasgroup = pdo_fetch("SELECT * FROM " . tablename(BEST_GROUP) . " WHERE weid = {$_W['uniacid']} AND admin = '{$openid}' AND cservicegroupid = {$cgroupid}");
		if (!empty($hasgroup)) {
			$resArr['error'] = 1;
			$resArr['hhhid'] = $hasgroup['id'];
			$resArr['msg'] = '您已加入该客服分组群聊！';
			echo json_encode($resArr);
			die;
		}
		$cgroup = pdo_fetch("SELECT * FROM " . tablename(BEST_CSERVICEGROUP) . " WHERE weid = {$_W['uniacid']} AND cangroup = 1 AND id = {$cgroupid}");
		if (empty($cgroup)) {
			$resArr['error'] = 1;
			$resArr['msg'] = '不存在该客服分组！';
			echo json_encode($resArr);
		...
		$cservicelist = pdo_fetchall("SELECT b.* FROM " . tablename('messikefu_kefuandgroup') . " as a," . tablename(BEST_CSERVICE) . " as b WHERE a.groupid = {$cgroupid} AND a.weid = {$_W['uniacid']} AND a.kefuid = b.id AND b.ctype = 1");
		if (empty($cservicelist)) {
			$resArr['error'] = 1;
			$resArr['msg'] = '该客服组没有客服成员！';
			echo json_encode($resArr);
			die;
		}
		$setting = $this->setting;
		$data1['weid'] = $_W['uniacid'];
		$data1['groupname'] = $cgroup['name'] . "讨论群";
		$data1['thumb'] = $cgroup['thumb'];
		$data1['admin'] = $openid;
		$data1['cservicegroupid'] = $cgroupid;
		$data1['time'] = TIMESTAMP;
		pdo_insert(BEST_GROUP, $data1);
		$ggid = pdo_insertid();
		$data2['weid'] = $_W['uniacid'];
		$data2['groupid'] = $ggid;
		$data2['openid'] = $openid;
		$data2['nickname'] = empty($_W['fans']['tag']['nickname']) ? '匿名用户' : $_W['fans']['tag']['nickname'];
		$data2['avatar'] = empty($_W['fans']['tag']['avatar']) ? tomedia($setting['defaultavatar']) : $_W['fans']['tag']['avatar'];
		$data2['type'] = 1;
		$data2['status'] = 1;
		$data2['intime'] = TIMESTAMP;
		pdo_insert(BEST_GROUPMEMBER, $data2);
		foreach ($cservicelist as $k => $v) {
			$datacc['weid'] = $_W['uniacid'];
			$datacc['groupid'] = $ggid;
			$datacc['openid'] = $v['content'];
			$datacc['nickname'] = $v['name'];
			$datacc['avatar'] = tomedia($v['thumb']);
			$datacc['type'] = 2;
			$datacc['status'] = 1;
			$datacc['intime'] = TIMESTAMP;
			pdo_insert(BEST_GROUPMEMBER, $datacc);
		}
		$resArr['error'] = 0;
		$resArr['ggid'] = $ggid;
		$resArr['msg'] = '创建群聊成功！';
	...
	public function doMobileZhuanjie()
	{
		...
		$setting = $this->setting;
		$touser = pdo_fetch("SELECT * FROM " . tablename(BEST_CSERVICE) . " WHERE weid = {$_W['uniacid']} AND content = '{$openid}'");
		...
		if ($setting['istplon'] == 1) {
			...
			if (!empty($tpllist)) {
				if ($tpllist['tplbh'] == 'OPENTM205211081') {
					$arrmsg = array('openid' => $data['content'], 'topcolor' => '#980000', 'first' => $tplcon, 'firstcolor' => '#990000', 'keyword1' => $data['nickname'], 'keyword1color' => '', 'keyword2' => date("Y-m-d H:i:s", TIMESTAMP), 'keyword2color' => '', 'keyword3' => $tplcon, 'keyword3color' => '', 'remark' => '', 'remarkcolor' => '', 'url' => $_W["siteroot"] . 'app/' . str_replace("./", "", $this->createMobileUrl("servicechat", array('toopenid' => $data['toopenid']))));
				} else {
					$arrmsg = array('openid' => $data['content'], 'topcolor' => '#980000', 'first' => $tplcon, 'firstcolor' => '#990000', 'keyword1' => '', 'keyword1color' => '', 'keyword2' => $tplcon, 'keyword2color' => '', 'remark' => '转接时间：' . date("Y-m-d H:i:s", TIMESTAMP), 'remarkcolor' => '', 'url' => $_W["siteroot"] . 'app/' . str_replace("./", "", $this->createMobileUrl("servicechat", array('toopenid' => $data['toopenid']))));
				}
				$this->sendtemmsg($tpllist['id'], $arrmsg);
			} else {
				$texturl = $_W["siteroot"] . 'app/' . str_replace("./", "", $this->createMobileUrl("servicechat", array('toopenid' => $data['toopenid'])));
				$texturl = "<a href='" . $texturl . "'>点击查看</a>";
				$concon = $tplcon . '。' . $texturl;
				$send['touser'] = $data['content'];
				$send['msgtype'] = 'text';
				$send['text'] = array('content' => urlencode($concon));
				$acc = WeAccount::create($_W['uniacid']);
				$res = $acc->sendCustomNotice($send);
	...
	public function doMobileAddchat()
	{
		...
		$jqr = pdo_fetch("SELECT * FROM " . tablename(BEST_JQR) . " WHERE weid = {$_W['uniacid']} AND title = '{$chatcontent}' AND kefuid = {$csrvice['id']} AND type = 0");
		$jqr2 = pdo_fetch("SELECT * FROM " . tablename(BEST_JQR) . " WHERE weid = {$_W['uniacid']} AND title like '%{$chatcontent}%' AND kefuid = {$csrvice['id']} AND type = 1");
		if (!empty($jqr) || !empty($jqr2)) {
			if (!empty($jqr)) {
				$datajqr['content'] = $jqr['huifu'];
			} else {
				$datajqr['content'] = $jqr2['huifu'];
			}
			$datajqr['openid'] = $csrvice['content'];
			$datajqr['nickname'] = $csrvice['name'];
			$datajqr['avatar'] = tomedia($csrvice['thumb']);
			$datajqr['toopenid'] = $_W['fans']['from_user'];
			$datajqr['time'] = TIMESTAMP;
			...
			if ($setting['istplon'] == 1 && $guotime > $setting['kefutplminute'] && $guotime2 > $setting['kefutplminute']) {
				...
				if (!empty($tpllist)) {
					if ($tpllist['tplbh'] == 'OPENTM202309749') {
						$arrmsg = array('openid' => $data['toopenid'], 'topcolor' => '#980000', 'first' => $data['nickname'] . '发起了咨询', 'firstcolor' => '#990000', 'keyword1' => $data['nickname'], 'keyword1color' => '', 'keyword2' => date("Y-m-d H:i:s", TIMESTAMP), 'keyword2color' => '', 'remark' => $tplcon, 'remarkcolor' => '', 'url' => $_W["siteroot"] . 'app/' . str_replace("./", "", $this->createMobileUrl("servicechat", array('toopenid' => $data['openid'], 'qudao' => $qudao, 'goodsid' => $goodsid))));
					} else {
	...
	public function doMobileAddchat2()
	{
		...
		if ($setting['istplon'] == 1 && $guotime > $setting['kefutplminute'] && $guotime2 > $setting['kefutplminute']) {
			...
			if (!empty($tpllist)) {
				if ($tpllist['tplbh'] == 'OPENTM205211081') {
					$arrmsg = array('openid' => $data['toopenid'], 'topcolor' => '#980000', 'first' => $data['nickname'] . '回复了您！', 'firstcolor' => '#990000', 'keyword1' => $data['nickname'], 'keyword1color' => '', 'keyword2' => date("Y-m-d H:i:s", TIMESTAMP), 'keyword2color' => '', 'keyword3' => $tplcon, 'keyword3color' => '', 'remark' => '', 'remarkcolor' => '', 'url' => $_W["siteroot"] . 'app/' . str_replace("./", "", $this->createMobileUrl("chat", array('toopenid' => $data['openid'], 'qudao' => $qudao, 'goodsid' => $goodsid))));
				} else {
